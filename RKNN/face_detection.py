...
DEVICE_COMPATTBLE_NODE = '/proc/device-tree-compatible'
...
class FaceDetector:
    ...
    def __init__(self, model_file):
        ...
        #self.rknn_lite.config([[127.5, 127.5, 127.5]], std_values=[[127.5, 127.5, 127.5]], target_platform='rk3588')
        self.rknn_lite.config(mean_values=[[0, 0, 0]], std_values=[[1, 1, 1]], target_platform='rk3588')
        ret =  self.rknn_lite.load_onnx(model="/home/jtl/playmate_robot/light_face_process/assets/face_detector.onnx")
        #ret = self.rknn_lite.build(do_quantization=True,dataset="dataset.txt")
        ret = self.rknn_lite.build(do_quantization=False)
        ret = self.rknn_lite.export_rknn("face_detector.rknn")
    ...
    def forward(self, img, threshold):
        ...
        predictions = self.rknn_lite.inference(inputs=inputs)
